...
const tokenModifiers = []; // Add modifiers if needed (e.g., 'declaration', 'readonly')
...
async function validateTextDocument(textDocument) {
    ...
    while ((funcMatch = functionCallRegex.exec(text)) !== null) {
        ...
        if (inString)
            continue; // Don't validate function calls found inside strings
        if (!funcInfo) {
                ...
                for (let i = 0; i < argCount; i++) {
                    ...
                    if (isLiteral(argText)) {
                        ...
                        if (literalType && !isTypeCompatible(literalType, expectedType)) {
                            diagnostics.push({
                                severity: node_1.DiagnosticSeverity.Warning, // Warning due to implicit conversions & basic check
                                range: argRange,
...
connection.languages.semanticTokens.on((params) => {
    ...
    const tokenPatterns = [
        ...
        {
            regex: /\b(parameters|variables|trigger|triggerBody|triggerOutputs|action|actions|item|items|workflow|result|if|equals|not|and|or|greater|less|true|false|null)\b/gi,
            type: "keyword",
    ...
    for (const pattern of tokenPatterns) {
        ...
        while ((match = pattern.regex.exec(text)) !== null) {
            ...
            const tokenStartIndex = pattern.type === "function"
            ...
            if (!tokenPositions.has(posKey)) {
                ...
                if (typeIndex !== -1) {
                    builder.push(startPos.line, startPos.character, tokenLength, typeIndex, 0);
...
function isTypeCompatible(literalType, expectedType) {
    ...
    if (expectedType.includes("|")) { // Handle union types simply for now
        ...
        if (allowedTypes.includes(literalType))
            return true;
        // Allow number-like for unions containing NumberLike or specific number types
        if ((literalType === "Integer" || literalType === "Float") && (allowedTypes.includes("NumberLike") || allowedTypes.includes("Number") || allowedTypes.includes("Integer") || allowedTypes.includes("Float")))
            return true;
        // Allow bool/number for string unions
        if (literalType !== "String" && allowedTypes.includes("String"))
            return true; // Allow implicit to string
    ...
    if (expectedType === "String" && (literalType === "Integer" || literalType === "Float" || literalType === "Boolean"))
        return true;
    // Integer -> Float? Yes.
    if (expectedType === "Float" && literalType === "Integer")
        return true;
    // Integer/Float -> NumberLike? Yes.
    if (expectedType === "NumberLike" && (literalType === "Integer" || literalType === "Float"))
        return true;
    ...
    for (let i = 0; i < argsText.length; i++) {
        ...
        if (!inSingleQuotes) {
            if (char === '(')
                ...
                curlyBraceLevel++; // Basic check for @{...}
            else if (char === '}')
                curlyBraceLevel--;
